            ...
            <div class="grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
                <div>
                    ...
                    <div class="mt-7 flex flex-col gap-3 sm:flex-row">
                        ...
                        @endphp
                        <a href="{{ $heroCtaUrl }}" @if($isHeroWa) target="_blank" rel="noopener" @endif data-track-click="cta_service_hero" data-track-label="{{ $service->name }}" class="inline-flex items-center justify-center gap-2 rounded-full {{ $isHeroWa ? 'bg-[#25D366]' : 'bg-brand-700' }} px-7 py-3.5 text-sm font-semibold text-white shadow-lg transition hover:brightness-95">
                            {{ $heroCtaText }}
                        </a>
        ...
        <div class="grid grid-cols-1 gap-10 lg:grid-cols-3">
            <div class="space-y-12 lg:col-span-2">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 border-y border-brand-100 py-6">
                    <x-public.share-buttons :title="$service->name" />
                    <span class="text-xs text-ink/40">Bagikan layanan ini</span>
                </div>
            </div>
            ...
            <aside class="space-y-6 lg:sticky lg:top-6 lg:self-start">
                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-ink/5">
                    ...
                    @endphp
                    <a href="{{ $ctaUrl }}" @if($isWa) target="_blank" rel="noopener" @endif data-track-click="cta_service_sidebar" data-track-label="{{ $service->name }}" class="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-brand-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-800">
                        {{ $ctaText }}
                    </a>
                    @if ($service->locations->isNotEmpty())
                    ...
                    <div class="rounded-2xl bg-brand-950 p-6 text-cream">
                        ...
                        <div class="mt-4 space-y-2">
                            @foreach ($service->locations as $location)
                                ...
                                <a href="{{ $locWa }}" target="_blank" rel="noopener" data-track-click="whatsapp_lokasi_service" data-track-label="{{ $service->name }} - {{ $location->name }}" class="flex items-center justify-between rounded-xl bg-white/10 px-4 py-3 text-sm font-medium transition hover:bg-white/20">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-800 via-brand-700 to-brand-900 px-6 py-14 text-center shadow-xl sm:px-12">
            ...
            <div class="relative">
                ...
                <div class="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                    ...
                    @endphp
                    <a href="{{ $bottomUrl }}" @if($bottomIsWa) target="_blank" rel="noopener" @endif data-track-click="cta_service_bottom" data-track-label="{{ $service->name }}" class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-7 py-3.5 text-sm font-semibold text-white shadow-lg transition hover:brightness-95 sm:w-auto">
