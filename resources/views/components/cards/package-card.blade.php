...
<div class="group flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-ink/5 transition hover:-translate-y-1 hover:shadow-lg {{ $hasPromo ? 'ring-2 ring-gold-400' : '' }}">
    <a href="{{ route('packages.show', $package) }}" class="relative aspect-[3/2] overflow-hidden">
        @if ($package->image)
    ...
    <div class="flex flex-1 flex-col p-5">
        <h3 class="font-display text-xl font-semibold text-brand-800"><a href="{{ route('packages.show', $package) }}" class="transition hover:text-brand-600">{{ $package->name }}</a></h3>
