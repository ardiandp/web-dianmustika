        ...
        <table class="min-w-full divide-y divide-ink/10 text-sm">
            <thead class="bg-ink/5">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center font-medium text-ink/60">Jumlah Artikel</th>
            ...
            <tbody class="divide-y divide-ink/10">
                @forelse ($categories as $category)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-center text-ink/60">{{ $category->articles_count }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.article-categories.edit', $category) }}" class="font-medium text-brand-700 hover:text-brand-800">Edit</a>
                            <form method="POST" action="{{ route('admin.article-categories.destroy', $category) }}" class="inline" onsubmit="return confirm('Hapus kategori ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-ink/50">Belum ada kategori artikel.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
