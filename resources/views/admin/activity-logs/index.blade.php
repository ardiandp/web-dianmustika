<x-layouts.admin title="Log Aktivitas">
    <x-admin.page-header title="Log Aktivitas" description="Riwayat perubahan, posting, hapus, serta login/logout. Hanya admin." />
    ...
    <div class="card card-outline card-primary">
        ...
        <div class="card-footer d-flex justify-content-between align-items-center">
            <small class="text-muted">Total: {{ $logs->count() }} log | Retensi 6 bulan.</small>
            <form method="POST" action="{{ route('admin.activity-logs.prune') }}" onsubmit="return confirm('Hapus semua log lebih dari 6 bulan?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-warning btn-sm"><i class="fas fa-broom mr-1"></i> Bersihkan &gt;6 Bulan</button>
            </form>
        </div>
    </div>
                    ...
                    @foreach ($logs as $log)
                        <tr>
                            ...
                            <td class="text-center">
                                @php $badge = match($log->action) { 'created' => 'badge-success', 'updated' => 'badge-warning', 'deleted' => 'badge-danger', 'login' => 'badge-primary', 'logout' => 'badge-secondary', 'login_failed' => 'badge-dark', default => 'badge-light', }; @endphp
                            ...
                            <td>
                                ...
                                @if ($log->changes)
                                    <button type="button" class="btn btn-link btn-sm p-0 ml-2" data-toggle="modal" data-target="#log-{{ $log->id }}"><i class="fas fa-eye"></i> Detail</button>
                                @endif
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Detail — {{ $log->description }}</h5>
